                                    ...
                                    )
{
    ...
    normalizedCountsStatisticsSmall <- lapply(normalizedCountsStatistics,function(i)
    {
      ...
      {
        ### In this modified version of the code, we add premature and mature signal
        ### for polysomal and cytoplasmic RNA contemplating nucleosomal premature RNA
        ### export and following processes.

        j[,grepl("^cym",colnames(j))] <- j[,grepl("^cym",colnames(j))]+j[,grepl("^cyp",colnames(j))]
        j[,grepl("^pym",colnames(j))] <- j[,grepl("^pym",colnames(j))]+j[,grepl("^pyp",colnames(j))]
                               ...
                               ,CVs)
{
  ...
  generated_gaussian <- lapply(seq_along(species), function(x){
    ...
    # Old code to deal with labeling times
    #  
    # if(length(V[grep(2,names(V))])>0){
    #   CVs[[x]] <- CVs[[x]][sort(names(CVs[[x]]))]
    #   new<-do.call(c,lapply((CVs[[x]][grep(0.33,names(CVs[[x]]))]),function(i){rep(i,2)}))
    #   names(new) <- setdiff(names(V),names(CVs[[x]]))
    #   CVs[[x]] <- c(CVs[[x]],new)
    #   CVs[[x]] <- CVs[[x]][sort(names(CVs[[x]]))]
    # }else if((length(V[grep(1,names(V))])>0)&(length(V[grep(2,names(V))])==0)){
    #   CVs[[x]] <- CVs[[x]][sort(names(CVs[[x]]))]
    #   new<-do.call(c,lapply((CVs[[x]][grep(0.33,names(CVs[[x]]))]),function(i){rep(i,1)}))
    #   names(new) <- setdiff(names(V),names(CVs[[x]]))
    #   CVs[[x]] <- c(CVs[[x]],new)
    #   CVs[[x]] <- CVs[[x]][sort(names(CVs[[x]]))]
    # }else{
                        ...
                        ,ratesOrder=c("k1","k2","k3","k4","k5","k11","k9","k6","k8","k7","k10"))
{
  ...
  # ord <- cutree(hclust(dist(cbind(ExampleDataBinned,inferedRatesBinned,inferedRatesBinned,inferedRatesBinned))),k = n_clust)
  # ord <- split(ord,ord)
  # ord <- ord[sort(sapply(ord,length),decreasing=TRUE,index.return=TRUE)[[2]]]
  # ord <- sapply(ord,function(i)names(i)[hclust(dist(cbind(ExampleDataBinned,inferedRatesBinned)[names(i),]))$ord])

  # ord2 <- unlist(sapply(seq_along(ord),function(i){rep(LETTERS[i],length(ord[[i]]))}))
  # names(ord2) <- unlist(ord,names)

  # rowAnnotation <- as.data.frame(list("Cluster"=ord2))

  # pheatmap(cbind(ExampleDataBinned,inferedRatesBinned)[names(ord2),]
                                    ...
                                    ,externalAnnotationColors=NULL)
{
  ...
  # ord <- cutree(hclust(dist(cbind(ExampleData,inferedRates,inferedRates,inferedRates))),k = n_clust)
  # ord <- split(ord,ord)
  # ord <- ord[sort(sapply(ord,length),decreasing=TRUE,index.return=TRUE)[[2]]]
  # ord <- sapply(ord,function(i)
  # {
  #   if(length(i)>1)
  #   {
  #     names(i)[hclust(dist(cbind(ExampleData,inferedRates)[names(i),]))$ord]
  #   }else{names(i)}
  # })
...
# TailsCharacterization <- function(txdb,clustering=NULL,obj_name,bams,nanopolishObjects,minoverlap_I,minoverlap_E,mergeReplicates,width,height,prematureSplit=FALSE){
#   if(mergeReplicates)
#   {
#     names(bams) <- gsub("1$","",gsub("2$","",names(bams)))
#     names(nanopolishObjects) <- gsub("1$","",gsub("2$","",names(nanopolishObjects)))
#   }

#   if(!identical(unique(names(bams)),unique(names(nanopolishObjects))))
#   {
#     print("Input object issues.")
#     stop()
#   }

#   outTmp <- lapply(unique(names(bams)),function(i)
#   {
#     print(i)

#     bamTmp <- bams[names(bams)==i]
#     tailTmp <- nanopolishObjects[names(nanopolishObjects)==i]
...
#   })
#   names(outTmp) <- unique(names(bams))
  
#   tailsTmp <- unlist(lapply(outTmp,function(i)list(sapply(i[[1]],median),sapply(i[[2]],median),sapply(i[[3]],median))),recursive=FALSE)

#   names(tailsTmp) <- gsub("1$","_In",names(tailsTmp))
#   names(tailsTmp) <- gsub("2$","_Ex",names(tailsTmp))
#   names(tailsTmp) <- gsub("3$","",names(tailsTmp))

#   if(prematureSplit)
#   {
#     tailsPlot <- tailsTmp[grepl("_In",names(tailsTmp))|grepl("_Ex",names(tailsTmp))]
#     pdf(paste0("polyATailLength_",obj_name,".pdf"),width=4,height=6)
#     par(mfrow=c(2,1))
#     boxplot(tailsPlot,varwidth=TRUE,outline=FALSE,las=2,col=2:3,ylab="PolyA tail length [bs]",main="")
#     legend("top",col=2:3,pch=15,legend=c("Intronic","Exonic"),box.lwd=NaN,ncol=2)
#     dev.off()
#   }else{
#     tailsPlot <- tailsTmp[!(grepl("_In",names(tailsTmp))|grepl("_Ex",names(tailsTmp)))]
#     pdf(paste0("polyATailLength_",obj_name,".pdf"),width=2.5,height=4)
#     #par(mfrow=c(2,1))
#     boxplot(tailsPlot,varwidth=TRUE,outline=FALSE,las=2,ylab="Median PolyA tail\nlength [bs]",main="")
#     dev.off()
#   }
...
#     par(mfrow=c(2,length(names(tailsTmp))/2))
#     for(j in names(tailsTmp))
#     {
#       l <- tailsTmp[[j]]
#       tailsTmp2 <- lapply(split(names(clustering),clustering),function(i)l[names(l)%in%i])
#       boxplot(tailsTmp2,varwidth=TRUE,outline=FALSE,las=2,ylab="[bs]",main=paste0("PolyA tail length - ",j))
#     }
#     dev.off()
#   }
                                  ...
                                  tailsPaths_treated,minoverlap_I,minoverlap_E,mergeReplicates,width,height)
{
  ...
  # pl <- ggplot(df,aes(x=Species,y=Values,fill=Condition)) + 
  #   geom_boxplot(outlier.shape = NA) +
  #   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  #         panel.background = element_blank(), axis.line = element_line(colour = "black"))
  # 
  # ggsave(paste0("foe.pdf"),pl,width=2.5, height=3)
...
## Goodness of fit
GoodnessOfFit <- function(inferedRates,expressionFlag,width=7,height=12,name="",lowSat=0,upSat=1)
{
...
simulatedRatesCorrelations <- function(object1,simObject,width=7,height=12,imageStructure=NULL,name=""
...
# differentialYields <- function(object1,object2,idx,boxPlots=FALSE,fileNameLabel="",height=6,width=8)
# {
#   commonGenesTmp <- intersect(rownames(object1$mean),rownames(object2$mean))
#   if(!(idx%in%commonGenesTmp|idx%in%rownames(object1$distributions)))
#   {
#     print("Not available gene and/or distributions!")
#     return(NULL)
#   }else{
#     outTmp <- sapply(names(object1$distributions),function(j)
#     {
#       j1 <- object1$distributions[[j]][idx,]
#       j2 <- object2$distributions[[j]][idx,]
      
#       WTmp <- wilcox.test(j1,j2)
#       KTmp <- ks.test(j1,j2)
#       c(WS=WTmp$statistic,WP=WTmp$p.value,KS=KTmp$statistic,KP=KTmp$p.value)
#     })

#     if(boxPlots)
#     {
#       object1Mean <- log10(object1$mean[idx,!grepl("0$",colnames(object1$mean))])
#       object2Mean <- log10(object2$mean[idx,!grepl("0$",colnames(object2$mean))])

#       object1Bottom <- log10(object1$bottom[idx,!grepl("0$",colnames(object1$bottom))])
#       object2Bottom <- log10(object2$bottom[idx,!grepl("0$",colnames(object2$bottom))])

#       object1Up <- log10(object1$up[idx,!grepl("0$",colnames(object1$up))])
#       object2Up <- log10(object2$up[idx,!grepl("0$",colnames(object2$up))])

#       minBoundary <- min(c(object1Bottom,object2Bottom,object1Mean,object2Mean)[is.finite(c(object1Bottom,object2Bottom,object1Mean,object2Mean))])
#       maxBoundary <- max(c(object1Up,object2Up,object1Mean,object2Mean)[is.finite(c(object1Up,object2Up,object1Mean,object2Mean))])

#         x <- matrix(NaN,nrow=6,ncol=length(object1Mean))
#         x[1,] <- object1Mean
#         x[3,] <- object2Mean
#         colTmp <- rep(1,nrow(x));colTmp[3] <- 2

#         x2 <- x
#         x2[x2==-Inf] <- minBoundary

#         labelsTmp <- c(sapply(names(object1Mean),function(i){c("",i,rep("",4))}))

#         y <- matrix(NaN,nrow=6,ncol=length(object1Bottom))
#         y[1,] <- object1Bottom
#         y[3,] <- object2Bottom

#         y2 <- y
#         y2[y2==-Inf] <- minBoundary

#         z <- matrix(NaN,nrow=6,ncol=length(object1Up))
#         z[1,] <- object1Up
#         z[3,] <- object2Up

#         z2 <- z
#         z2[z2==-Inf] <- minBoundary
...
#         plot(c(x2),pch=16,col=colTmp,ylim=c(minBoundary,maxBoundary),xaxt = 'n',xlab="",main=idx,ylab="Log10 [fg/M cells]")
#         axis(1, at=seq_along(x2), labels = FALSE, lwd.ticks = 1*(labelsTmp!=""))
#         text(seq_along(x2), par("usr")[3] - 0.2, labels = labelsTmp, srt = 90, pos = 1, xpd = TRUE)
#         arrows(x0=seq_along(x2),y0=x2,x1=seq_along(x2),y1=y2,length=0,col=colTmp,lty=lineTypeTmp)
#         arrows(x0=seq_along(x2),y0=x2,x1=seq_along(x2),y1=z2,length=0,col=colTmp)
#         dev.off()
#     }
#     return(outTmp)
#   }
...
### Code to remove genes according to the intronic signal, to be included in the final version of the code.
#
# if(!is.null(cytPTh))
# {
#   # cytoplasmicExpressedGenes <- lapply(unique(sapply(strsplit(names(cytoplasmic),"_"),"[[",1)),function(j)
#   # {
#   #   cytoplasmicExpressedGenesTmp <- lapply(cytoplasmic[grepl(paste0("^",j,"_"),names(cytoplasmic))],function(i)
#   #   {
#   #   unique(rownames(i)[(i[,paste0("cyn",labelingTime)]>0)
#   #                     &(i[,paste0("cyp",labelingTime)]>0)])
#   #   })
#   #   cytoplasmicExpressedGenesTmp <- unique(unlist(cytoplasmicExpressedGenesTmp))
#   # })
#   # cytoplasmicExpressedGenes <- names(which(table(unlist(cytoplasmicExpressedGenes))==length(cytoplasmicExpressedGenes)))

#   cytoplasmicExpressedGenes <- lapply(names(cytoplasmic),function(j)
#   {
#     i <- cytoplasmic[[j]]
#     cytoplasmicExpressedGenesTmp <- rownames(i)[(i[,paste0("cyn",labelingTime)]>0)&(i[,paste0("cyp",labelingTime)]>0)]
#   })
#   names(cytoplasmicExpressedGenes) <- names(cytoplasmic)

#   expressedGenes <- lapply(seq_along(cytoplasmicExpressedGenes),function(i)
#   {
#     expressedGenes <- intersect(intersect(chromatinExpressedGenes[[i]],nucleoplasmicExpressedGenes[[i]]),cytoplasmicExpressedGenes[[i]])
#     print(paste0("Expressed genes condition ",i,":",length(expressedGenes)))
#     expressedGenes
#   })

#   print(paste0("Exclusion of genes according to the Cytoplasmic premature RNA relative expression level; threshold ",cytPTh))
#   chromatinMat <- sapply(seq_along(chromatin),function(j)
#   {
#     i <- chromatin[[j]]
#     outTmp <- rep(0,length(expressedGenes[[j]]))
#     names(outTmp) <- expressedGenes[[j]]
#     genesTmp <- intersect(rownames(i),expressedGenes[[j]])
#     # outTmp[genesTmp] <- i[genesTmp,"chpt"]
#     outTmp[genesTmp] <- i[genesTmp,"chpt"]/(i[genesTmp,"chpt"]+i[genesTmp,"chmt"])
#     outTmp
#   })

#   nucleoplasmicMat <- sapply(seq_along(nucleoplasmic),function(j)
#   {
#     i <- nucleoplasmic[[j]]
#     outTmp <- rep(0,length(expressedGenes[[j]]))
#     names(outTmp) <- expressedGenes[[j]]
#     genesTmp <- intersect(rownames(i),expressedGenes[[j]])
#     # outTmp[genesTmp] <- i[genesTmp,"npt"]
#     outTmp[genesTmp] <- i[genesTmp,"npt"]/(i[genesTmp,"npt"]+i[genesTmp,"nmt"])
#     outTmp
#   })

#   cytoplasmicPMat <- sapply(seq_along(cytoplasmicP),function(j)
#   {
#     i <- cytoplasmicP[[j]]
#     outTmp <- rep(0,length(expressedGenes[[j]]))
#     names(outTmp) <- expressedGenes[[j]]
#     genesTmp <- intersect(rownames(i),expressedGenes[[j]])
#     # outTmp[genesTmp] <- i[genesTmp,"cypt"]
#     outTmp[genesTmp] <- i[genesTmp,"cypt"]/(i[genesTmp,"cypt"]+i[genesTmp,"cymt"])
#     outTmp
#   })

#   expressedGenesNoCyp <- lapply(seq_along(chromatinMat),function(j)
#   {
#     chromatinFMat <- (chromatinMat[[j]]/cytoplasmicPMat[[j]])
#     chromatinFMat[is.nan(chromatinFMat)] <- Inf
    
#     nucleoplasmicFMat <- (nucleoplasmicMat[[j]]/cytoplasmicPMat[[j]])
#     nucleoplasmicFMat[is.nan(nucleoplasmicFMat)] <- Inf
    
#     ## Expressed genes without cytoplasmic premature RNA
#     # expressedGenesNoCyp <- expressedGenes[apply(cytoplasmicPMat,1,sum)==0]
#     # expressedGenesNoCyp <- expressedGenes[apply(chromatinFMat>cytPTh,1,all)|apply(nucleoplasmicFMat>cytPTh,1,all)]
#     expressedGenesNoCyp <- expressedGenes[[j]][chromatinFMat>cytPTh|chromatinFMat>cytPTh]

#     print(paste0("Expressed genes without cytoplasmic premature RNA condition ",j,":",length(expressedGenesNoCyp)))
#     expressedGenesNoCyp
#   })
# }else{
#   print(paste0("Exclusion of Cytoplasmic premature RNA reads."))
  ...
  ### This is not working anymore because the code assumes that all the chr samples are replicates.
  ### It is not suitable for the analysis of different experiments.
  #
  # meanNormalizedCountsNoCypYesChpNp <- lapply(normalizedCountsNoCypYesChpNp,function(i)Reduce("+", i)/length(i))
  # meanNormalizedCountsNoCypNpYesChp <- lapply(normalizedCountsNoCypNpYesChp,function(i)Reduce("+", i)/length(i))
  # meanNormalizedCountsNoCyp <- lapply(normalizedCountsNoCyp,function(i)Reduce("+", i)/length(i))

  #  sdNormalizedCountsNoCypYesChpNp <- lapply(names(normalizedCountsNoCypYesChpNp),function(n)
  # {
  #     j <- lapply(names(normalizedCountsNoCypYesChpNp[[n]]),function(r)
  #     {
  #         (normalizedCountsNoCypYesChpNp[[n]][[r]] - meanNormalizedCountsNoCypYesChpNp[[n]])^2
  #     })
  #     sqrt(Reduce("+",j)/(length(j)-1))
  # })
  # names(sdNormalizedCountsNoCypYesChpNp) <- names(normalizedCountsNoCypYesChpNp)

 #  # sdNormalizedCountsNoCypNpYesChp <- lapply(names(normalizedCountsNoCypNpYesChp),function(n)
  # {
  #     j <- lapply(names(normalizedCountsNoCypNpYesChp[[n]]),function(r)
  #     {
  #         (normalizedCountsNoCypNpYesChp[[n]][[r]] - meanNormalizedCountsNoCypNpYesChp[[n]])^2
  #     })
  #     sqrt(Reduce("+",j)/(length(j)-1))
  # })
  # names(sdNormalizedCountsNoCypNpYesChp) <- names(normalizedCountsNoCypNpYesChp)

 #  # sdNormalizedCountsNoCypNpChp <- lapply(names(normalizedCountsNoCypNpChp),function(n)
  # {
  #     j <- lapply(names(normalizedCountsNoCypNpChp[[n]]),function(r)
  #     {
  #         (normalizedCountsNoCypNpChp[[n]][[r]] - meanNormalizedCountsNoCypNpChp[[n]])^2
  #     })
  #     sqrt(Reduce("+",j)/(length(j)-1))
  # })
  # names(sdNormalizedCountsNoCypNpChp) <- names(normalizedCountsNoCypNpChp)

  # ############### CORRELATION BETWEEN SAMPLES ##################
# CorrelationBtwSamples <- function(object
#                                  ,width
#                                  ,height
#                                  ,obj_name)
# {
...
#   InferedData <- object$inferedData
#   to_remove <- colnames(InferedData)[grep("0$",colnames(InferedData))]
#   to_keep <- setdiff(colnames(InferedData),to_remove)
#   
#   pdf(paste0("DataCorrelations",obj_name,".pdf"),width=width,height=height)
#   par(mfrow=c(length(to_keep)/3,3))
#   for(i in to_keep)
#   {
#     corTmp <- tryCatch(round(cor(log10(ExampleData[,i]),log10(InferedData[,i]),method="s",use="c"),2),error=function(e)NaN)
#     if(is.finite(corTmp))
#     {
#       smoothScatter(log10(InferedData[,i]),log10(ExampleData[,i]),xlab="Inferred",ylab="Experimental",main=i)
#       abline(0,1)
#       legend("bottomright",legend=corTmp)
#     }
#   }
#   dev.off()
# }
...
#   metrics_list <- list()
#   titles <- colnames(Metrics)
#   
#   for (i in 1:ncol(Metrics)){
#     
#     metrics_list[[i]] <- ggplot(Metrics, aes(x = Metrics[,i])) +
#       geom_histogram(aes(y = ..density..), fill=myPalette[i]) +
#       geom_density(alpha=.4, fill="grey") +
#       theme_classic() +
#       labs(x = colnames(Metrics)[i], y="Distribution") +
#       ggtitle(paste0("Distribution of",titles[i])) +
#       theme(plot.title = element_text(size = 15,hjust = 0.5))
#   }
#   
#   img <- do.call("grid.arrange", c(metrics_list, ncol = ncol(Metrics))) 
#   ggsave(filename=paste0(saving_path,"distributions_metrics.pdf"),plot = img ,width=width,height=height)
# 
# }

############# HEATMAP ################
# HeatmapPlot <- function(object
#                        ,object2=NULL
#                        ,upSat
#                        ,obj_name
#                        ,logBool)
...
#   if(!is.null(object2))
#   {
#     ExampleData2 <- object2$expressionData
#     inferedRates2 <- object2$inferedRates
#   }
# 
#   inferedRatesNorm <- sapply(1:ncol(inferedRates),function(j)
#   {
#     i <- inferedRates[,j]
#     if(!is.null(object2))
#     {
#       l <- c(i,inferedRates2[,j])
#       upB <- quantile(l,upSat)
#     }else{
#       upB <- quantile(i)
#     }
#   
...
#     i
#   })
#   colnames(inferedRatesNorm) <- colnames(inferedRates)
# 
#   SpeciesNorm <- sapply(1:ncol(ExampleData),function(j)
#   {
#     i <- ExampleData[,j]
#     if(!is.null(object2))
#     {
#       l <- c(i,ExampleData2[,j])
#       upB <- quantile(l,upSat)
#     }else{
#       upB <- quantile(i)
#     }
...
#     i
#   })
#   colnames(SpeciesNorm) <- colnames(ExampleData)
# 
#   if(!is.null(object2))
#   {
#     inferedRatesNorm2 <- sapply(1:ncol(inferedRates2),function(j)
#     {
#       i <- inferedRates2[,j]
#       if(!is.null(object2))
#       {
#         l <- c(i,inferedRates[,j])
#       }
# 
#       upB <- quantile(l,upSat)
#     
...
#       i
#     })
#     colnames(inferedRatesNorm2) <- colnames(inferedRates2)
# 
#     SpeciesNorm2 <- sapply(1:ncol(ExampleData2),function(j)
#     {
#       i <- ExampleData2[,j]
#       if(!is.null(object2))
#       {
...
#       i
#     })
#     colnames(SpeciesNorm2) <- colnames(ExampleData2)
#     ord <- hclust(dist(cbind(SpeciesNorm,inferedRatesNorm,SpeciesNorm2,inferedRatesNorm2)))$ord
#   }
# 
#   to_hold <- colnames(ExampleData)[grep("t$",colnames(ExampleData))]
#   SpeciesNorm <- SpeciesNorm[,colnames(SpeciesNorm)%in%to_hold]
# 
#   if(is.null(object2))
#   {
#     pheatmap(cbind(SpeciesNorm[,to_hold],inferedRatesNorm),cluster_cols = FALSE,show_rownames = FALSE,color = colorRampPalette(c("white","pink","red"))(41),
#              filename = paste0("heatmapBIG_",obj_name,".pdf"))
#     pheatmap(inferedRatesNorm,cluster_cols = FALSE,show_rownames = FALSE, color = colorRampPalette(c("white","red"))(41),filename = paste0("heatmap_",obj_name,".pdf"))
#   }else{
#     pheatmap(cbind(SpeciesNorm[ord,to_hold],inferedRatesNorm[ord,]),cluster_rows = FALSE,cluster_cols = FALSE,show_rownames = FALSE,color = colorRampPalette(c("white","pink","red"))(41),
#              filename = paste0("heatmapBIG1_",obj_name,".pdf"))
#     pheatmap(inferedRatesNorm[ord,],cluster_rows = FALSE,cluster_cols = FALSE,show_rownames = FALSE, color = colorRampPalette(c("white","red"))(41),filename = paste0("heatmap1_",obj_name,".pdf"))
# 
#     pheatmap(cbind(SpeciesNorm2[ord,to_hold],inferedRatesNorm2[ord,]),cluster_rows = FALSE,cluster_cols = FALSE,show_rownames = FALSE,color = colorRampPalette(c("white","pink","red"))(41),
#              filename = paste0("heatmapBIG2_",obj_name,".pdf"))
#     pheatmap(inferedRatesNorm2[ord,],cluster_rows = FALSE,cluster_cols = FALSE,show_rownames = FALSE, color = colorRampPalette(c("white","red"))(41),filename = paste0("heatmap2_",obj_name,".pdf"))
#   }
# }
